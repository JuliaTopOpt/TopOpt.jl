...
@testset "ComplianceFun minimization with fixed elements" begin
    ...
    @test get_free_variable_count(free_to_full_proj) ==
    ...
    Nonconvex.addvar!(
        model,
        zeros(get_free_variable_count(free_to_full_proj)),
        ones(get_free_variable_count(free_to_full_proj));
        init=copy(x0_free),
    )
