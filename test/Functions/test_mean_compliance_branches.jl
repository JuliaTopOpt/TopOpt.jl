...
# Test the conditional branches in MeanComplianceFun constructor
# These tests specifically target the branching logic in lines ~18-43 of mean_compliance.jl

Random.seed!(42)

@testset "MeanComplianceFun Constructor Branches" begin
    ...
    @testset "sample_once parameter handling" begin
        ...
        mc = MeanComplianceFun(problem, solver; method=:trace, nv=3, sample_once=true)
        @test mc.method.sample_once == true

        # Test sample_once=false for TraceEstimationMean
        mc = MeanComplianceFun(problem, solver; method=:trace, nv=3, sample_once=false)
        @test mc.method.sample_once == false

        # Test sample_once=true for TraceEstimationSVDMean
        mc = MeanComplianceFun(problem, solver; method=:svd_trace, nv=3, sample_once=true)
        @test mc.method.sample_once == true

        # Test sample_once=false for TraceEstimationSVDMean
        mc = MeanComplianceFun(problem, solver; method=:svd_trace, nv=3, sample_once=false)
        @test mc.method.sample_once == false
    ...
    @testset "Both branches produce valid function values" begin
        ...
        mc_trace = MeanComplianceFun(problem, solver_trace; method=:trace, nv=10)
    ...
    @testset "Code duplication verification - both branches have identical V handling" begin
        ...
        solver1 = FEASolver(DirectSolver, problem; xmin=0.01, penalty=PowerPenaltyFun(2.0))
        mc_trace1 = MeanComplianceFun(problem, solver1; method=:trace)
        mc_svd1 = MeanComplianceFun(problem, solver1; method=:svd_trace)
        @test size(mc_trace1.method.V, 2) == size(mc_svd1.method.V, 2) == 1

        # Test case 2: V === nothing, nv=5
        solver2 = FEASolver(DirectSolver, problem; xmin=0.01, penalty=PowerPenaltyFun(2.0))
        mc_trace2 = MeanComplianceFun(problem, solver2; method=:trace, nv=5)
        mc_svd2 = MeanComplianceFun(problem, solver2; method=:svd_trace, nv=5)
        ...
        solver3 = FEASolver(DirectSolver, problem; xmin=0.01, penalty=PowerPenaltyFun(2.0))
        mc_trace3 = MeanComplianceFun(problem, solver3; method=:trace, V=V_trace)
        mc_svd3 = MeanComplianceFun(problem, solver3; method=:svd_trace, V=V_svd)
        ...
        solver4 = FEASolver(DirectSolver, problem; xmin=0.01, penalty=PowerPenaltyFun(2.0))
        mc_trace4 = MeanComplianceFun(problem, solver4; method=:trace, V=V_trace, nv=4)
        mc_svd4 = MeanComplianceFun(problem, solver4; method=:svd_trace, V=V_svd, nv=4)
