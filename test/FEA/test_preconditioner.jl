...
@testset "CG Solvers with Preconditioner" begin
    @testset "CGAssemblySolver with diagonal preconditioner" begin
        ...
        # but is NOT === identity
        dummy_prec = Preconditioners.DiagonalPreconditioner(
            ones(Float64, ndofs(problem.ch.dh))
        )
        ...
        solver_prec = FEASolver(
            CGAssemblySolver, problem; abstol=1e-7, preconditioner=dummy_prec
        )

        # Create solver with identity preconditioner for comparison
        solver_identity = FEASolver(
            CGAssemblySolver, problem; abstol=1e-7, preconditioner=identity
        )
    ...
    @testset "CGMatrixFreeSolver with diagonal preconditioner" begin
        ...
        # Create solver with preconditioner
        solver_prec = FEASolver(
            CGMatrixFreeSolver, problem; abstol=1e-7, preconditioner=dummy_prec
        )

        # Create solver with identity preconditioner for comparison
        solver_identity = FEASolver(
            CGMatrixFreeSolver, problem; abstol=1e-7, preconditioner=identity
        )
    ...
    @testset "CG solvers with non-trivial diagonal preconditioner" begin
        ...
        solver_assembly = FEASolver(
            CGAssemblySolver, problem; abstol=1e-7, preconditioner=varying_prec
        )
        ...
        solver_mf = FEASolver(
            CGMatrixFreeSolver, problem; abstol=1e-7, preconditioner=varying_prec2
        )
        solver_mf.vars .= 0.5
        solver_mf()
    ...
    @testset "Preconditioner comparison with DirectSolver" begin
        ...
        solver_cg = FEASolver(
            CGAssemblySolver, problem; abstol=1e-9, preconditioner=dummy_prec
        )
        solver_cg.vars .= 0.5
        solver_cg()
