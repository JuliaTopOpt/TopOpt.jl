...
function get_tol(asimp::AdaptiveSIMP{T}) where {T}
    ...
    if p >= asimp.pfinish - sqrt(eps(T))
        ...
        #=
        c_trace = asimp.simp.optimizer.obj.topopt_trace.c_hist
        n = length(c_trace)
        if n >= 10
            l = sum(@view c_trace[end-9:end-5])
            h = sum(@view c_trace[end-4:end])
            change = abs(l-h)/h
            progress = 1 - clamp(change, 0, 1)
        else
            progress = zero(T)
        end
        =#
