...
An abstract heat transfer topology optimization problem. All subtypes must have the following fields:
- `ch`: a `Ferrite.ConstraintHandler` struct with temperature DOFs (1 DOF per node)
- `metadata`: Metadata having various cell-node-dof relationships
- `black`: a `BitVector` of length equal to the number of elements where `black[e]` is 1 iff the `e`^th element must be part of the final design
- `white`:  a `BitVector` of length equal to the number of elements where `white[e]` is 1 iff the `e`^th element must not be part of the final design
- `varind`: an `AbstractVector{Int}` of length equal to the number of elements where `varind[e]` gives the index of the decision variable corresponding to element `e`.
...
struct HeatConductionProblem{dim, T, N, M} <: HeatTransferTopOptProblem{dim, T}
    ...
    ch::ConstraintHandler{<:DofHandler{dim, <:Cell{dim, N, M}, T}, T}
    heat_source::T          # volumetric heat generation
    black::AbstractVector
    white::AbstractVector
    varind::AbstractVector{Int}
    metadata::Metadata
...
A steady-state heat conduction problem with uniform heat generation and temperature boundary conditions.
"""
...
"""
    HeatConductionProblem(::Type{Val{CellType}}, nels::NTuple{dim, Int}, sizes::NTuple{dim}, k=1.0, heat_source=1.0; Tleft=0.0, Tright=0.0) where {dim, CellType}

Creates a heat conduction problem on a rectangular domain with:
- Temperature fixed at left boundary (T = Tleft)
- Temperature fixed at right boundary (T = Tright)
- Uniform heat generation throughout domain
- Thermal conductivity k (will be penalized in optimization)
