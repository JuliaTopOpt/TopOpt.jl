...
Base.show(::IO, ::MIME{Symbol("text/plain")}, ::PointLoadCantilever) =
    println("TopOpt point load cantilever beam problem")
...
Base.show(::IO, ::MIME{Symbol("text/plain")}, ::HalfMBB) =
    println("TopOpt half MBB problem")
...
Base.show(::IO, ::MIME{Symbol("text/plain")}, ::TieBeam) =
    println("TopOpt tie-beam problem")
...
getpressuredict(p::TieBeam{T}) where {T} =
    Dict{String,T}("rightload" => 2 * p.force, "bottomload" => -p.force)
