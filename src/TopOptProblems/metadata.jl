...
function Metadata(dh::DofHandler{dim}) where {dim}
    ...
    return meta = Metadata(cell_dofs, dof_cells, node_cells, node_dofs)
end
...
function get_dof_cells_matrix(dh, cell_dofs)
    dof_cells_vecofvecs = [Vector{Tuple{Int,Int}}() for i = 1:ndofs(dh)]
    l = 0
    for cellidx = 1:size(cell_dofs, 2)
        for localidx = 1:size(cell_dofs, 1)
...
function get_node_first_cells(dh)
    ...
    for cellidx = 1:getncells(dh.grid)
...
function get_node_cells(dh)
    node_cells_vecofvecs = [Vector{Tuple{Int,Int}}() for i = 1:ndofs(dh)]
    l = 0
...
function get_node_dofs(dh::DofHandler)
    ...
    for field = 1:nfields
        ...
        for (cellidx, cell) in enumerate(dh.grid.cells)
            ...
            for idx = 1:min(interpol_points, length(cell.nodes))
                node = cell.nodes[idx]
                if !visited[node]
