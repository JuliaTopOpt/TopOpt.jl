...
function SigmoidContinuation{T}(;
    c::T=T(0.1), start::T=T(1), finish::T=T(5), steps::Int=30, min::T=-Inf
) where {T}
    a = 1 - T(finish - start) / finish / (ℯ^(-c) - ℯ^(-steps * c)) * ℯ^(-c)
    b = T(finish - start) / finish / (ℯ^(-c) - ℯ^(-steps * c))
    return SigmoidContinuation(a, b, c, steps, min)
end
function Base.iterate(s::SigmoidContinuation, x=1)
    (x > s.length) && return nothing
    return max(1 / (s.a + s.b * ℯ^(-s.c * x)), s.min), x + 1
end
...
function CubicSplineContinuation{T}(;
    b::T=T(0.5), start::T=T(1), finish::T=T(5), steps::Int=30, min::T=-Inf
) where {T}
    a = (finish - start) / (steps^3 * (1 - b)^3 - (1 - b * steps)^3)
    c = start - a * (1 - b * steps)^3
    return CubicSplineContinuation(a, b, c, steps, min)
end

function Base.iterate(s::CubicSplineContinuation, x=1)
    x > s.length && return nothing
    return max(s.a * (x - s.b * s.length)^3 + s.c, s.min), x + 1
end
Base.length(s::CubicSplineContinuation) = s.length
function (s::CubicSplineContinuation{T})(x) where {T}
    return max(s.a * (T(x) - s.b * s.length)^3 + s.c, s.min)
end
...
function PowerContinuation{T}(;
    b=T(2), start=T(1), finish=T(5), steps::Int=30, min=-T(Inf)
) where {T}
    a = (finish - start) / max(T(1), steps^b - 1)
    c = start - a
    return PowerContinuation(a, b, c, steps, min)
end
function Base.iterate(s::PowerContinuation, x=1)
    x > s.length && return nothing
    return max(s.a * x^s.b + s.c, s.min), x + 1
end
...
function ExponentialContinuation{T}(;
    b::T=T(0.1), start::T=T(1), finish::T=T(5), steps::Int=30, min::T=-Inf
) where {T}
    a = (finish - start) / max(T(1), ℯ^(b * steps) - ℯ^(b))
    c = start - a * ℯ^(b)
    return ExponentialContinuation(a, b, c, steps, min)
end
function Base.iterate(s::ExponentialContinuation, x=1)
    x > s.length && return nothing
    return max(s.a * ℯ^(s.b * x) + s.c, s.min), x + 1
end
...
function LogarithmicContinuation{T}(;
    b::T=T(1), start::T=T(1), finish::T=T(5), steps::Int=30, min::T=-Inf
) where {T}
    a = (finish - start) / max(T(1), log(b * steps) - log(b))
    c = start - a * log(b)
    return LogarithmicContinuation(a, b, c, steps, min)
end
function Base.iterate(s::LogarithmicContinuation, x=1)
    x > s.length && return nothing
    return max(s.a * log(s.b * x) + s.c, s.min), x + 1
end
